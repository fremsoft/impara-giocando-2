...
void tftBuffEndScreen(void) {
  ...
  if ( ( (actualScreen == 7) || (actualScreen == 8) || (actualScreen == 9) )
    ...
    // HO PREMUTO SETTINGS
    currentElement = -1;  // Si puo' anche non mettere (vedi else)
    nextScreen = 6;
  } 
    ...
    if (url.length() > 0) { nextString = url; nextString.remove(url.length() - 1); }
      ...
      && ((touchX > 60-20) && (touchX < 60+20) && (touchY > 120-20) && (touchY < 120+20)) ) {
    // HO PREMUTO SUMMER TIME
    if ( currentElement < 0 ) { 
      setOraLegale( !getOraLegale() );
      // disattivo il currentElement ed evito che si inneschi un loop
      currentElement = 1;
    }
  }
