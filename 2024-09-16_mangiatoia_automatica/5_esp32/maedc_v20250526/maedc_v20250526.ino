...
// https://youtube.com/live/AgRAR5sBxQc
...
// Da oggi si può spedire online lo stato della #MAEDC
// e si può controllare se e' stato chiesto di erogare da remoto 
// (via Web)
...
uint16_t grammiEffettivamenteErogati; 
String   dataOraUltimaErogazione;
...
void setup() {
  ...
  pulsante_refill = digitalRead(INPUT_REFILL);
  ...
  grammiDaErogare = 0;
  grammiEffettivamenteErogati = 0;
  dataOraUltimaErogazione = "";
...
void loop() {
  ...
  switch ( actualScreen ) {
    ...
    case 1 : // schermata principale
      ...
      if ((minuti_day_old != minuti_now) && (minuti_day_old > 0)) {
        if (sendStatusWeb() != 0) {
          // e' ora di mangiare (fuori pasto)
          actualScreen = nextScreen = 3;
        }
      }
      ...
      if (
        ...
        if (statoErogazioneCibo == 2)  {
          ...
          if ((p < grammiDaErogare) && (p < MAX_PESO_PER_PESATA_G) && (!timeout_erogazione_expired)) {
            ...
            grammiEffettivamenteErogati += p;
            dataOraUltimaErogazione = getDataOra();
...
prepareEmail("Situazione #MAEDC", 
      "<p>Ciao " MAIL_TO_NAME ", <br>\r\n"
      "la #MAEDC non &egrave; riuscita a servire il pasto!</p>" 
      /* + dataEOra */ );
  ...
  if ( digitalRead(INPUT_REFILL) != pulsante_refill) {
    String dataEOra = "(no data e ora)";
    struct tm timeinfo;
    if (getLocalTime(&timeinfo)) {
      char buffer[32];
      strftime(buffer, 32, "%d/%m/%Y %H:%M:%S", &timeinfo);
      dataEOra = String(buffer);
    }

    // E' cambiato lo stato del pulsante di refill
    pulsante_refill = digitalRead(INPUT_REFILL);
    delay(100);  // antirimbalzo

    if (pulsante_refill == LOW) {
      // pulsante premuto
      prepareEmail("Situazione #MAEDC", 
      "<p>Ciao " MAIL_TO_NAME ", <br>\r\n"
      "la #MAEDC &egrave; stata riempita!</p>" 
      + dataEOra );
    }
    else {
      // pulsante rilasciato
      prepareEmail("Situazione #MAEDC", 
      "<p>Ciao " MAIL_TO_NAME ", <br>\r\n"
      "la #MAEDC <b>&egrave; quasi vuota</b>!</p>"
      + dataEOra );
    }
  }
