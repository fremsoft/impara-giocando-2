void polarToRect(int alfa1024, int beta1024, int gamma1024, float *x, float *y, float *z) {

  /* sulla base della geometria di Robot RollArm, determino i gradi di ogni servo */
  float alfaDeg, betaDeg, gammaDeg;
  alfaDeg = map(alfa1024, 0, 1023, 18000, 0) / 100.0;
  betaDeg = map(beta1024, 0, 1023, -9000, 9000) / 100.0;
  gammaDeg = map(gamma1024, 0, 1023, 18000, 0) / 100.0;

  /* trasformo da gradi a radianti */
  float alfaRad, betaRad, gammaRad;
  alfaRad = alfaDeg * PI / 180.00;
  betaRad = betaDeg * PI / 180.00;
  gammaRad = gammaDeg * PI / 180.00;

  /* origine del Robot */
  float xO, yO, zO;
  xO = 0;
  yO = 0;
  zO = 0;

  /* orogine del primo giunto */
  float xO1, yO1, zO1;
  xO1 = xO + 0;
  yO1 = yO + 0;
  zO1 = zO + 64;

  /* origine del secondo giunto */
  float xO2, yO2, zO2;
  xO2 = xO1 + 60 * cos(alfaRad) * cos(betaRad);
  yO2 = yO1 + 60 * cos(alfaRad) * sin(betaRad);
  zO2 = zO1 + 60 * sin(alfaRad);

  /* estremità del Robot RollArm */
  float xP, yP, zP;
  xP = xO2 + 110 * cos(alfaRad - gammaRad) * cos(betaRad);
  yP = yO2 + 110 * cos(alfaRad - gammaRad) * sin(betaRad);
  zP = zO2 + 110 * sin(alfaRad - gammaRad);

  *x = xP; *y = yP; *z = zP;
}

void polarToRectSonar (int alfa1024, int beta1024, int gamma1024, 
                       float *x, float *y, float *z, float *teta, float *beta) {

  /* sulla base della geometria di Robot RollArm, determino i gradi di ogni servo */
  float alfaDeg, betaDeg, gammaDeg;
  alfaDeg = map(alfa1024, 0, 1023, 18000, 0) / 100.0;
  betaDeg = map(beta1024, 0, 1023, -9000, 9000) / 100.0;
  gammaDeg = map(gamma1024, 0, 1023, 18000, 0) / 100.0;

  /* trasformo da gradi a radianti */
  float alfaRad, betaRad, gammaRad;
  alfaRad = alfaDeg * PI / 180.00;
  betaRad = betaDeg * PI / 180.00;
  gammaRad = gammaDeg * PI / 180.00;

  /* origine del Robot */
  float xO, yO, zO;
  xO = 0;
  yO = 0;
  zO = 0;

  /* origine del primo giunto */
  float xO1, yO1, zO1;
  xO1 = xO + 0;
  yO1 = yO + 0;
  zO1 = zO + 64;

  /* origine del secondo giunto */
  float xO2, yO2, zO2;
  xO2 = xO1 + 60 * cos(alfaRad) * cos(betaRad);
  yO2 = yO1 + 60 * cos(alfaRad) * sin(betaRad);
  zO2 = zO1 + 60 * sin(alfaRad);

  /* coordinate x,y,z del sonar */
  float xS, yS, zS;
  xS = xO2 + 40 * cos(alfaRad - gammaRad) * cos(betaRad) 
           - 50 * sin(alfaRad - gammaRad) * cos(betaRad);
  yS = yO2 + 40 * cos(alfaRad - gammaRad) * sin(betaRad)
           - 50 * sin(alfaRad - gammaRad) * sin(betaRad);
  zS = zO2 + 40 * sin(alfaRad - gammaRad)
           + 50 * cos(alfaRad - gammaRad);
  
  *x = xS; 
  *y = yS; 
  *z = zS;

  *teta = alfaRad - gammaRad;
  *beta = betaRad;
}

void writeServo() {

  Servo_1.write(map(alfa1024, 0, 1023, 0, 180)       + OFFSET_ALFA); 
  Servo_0.write(map(beta1024, 0, 1023, 0, 180)       + OFFSET_BETA); 
  Servo_2.write(map(gamma1024, 0, 1023, 0, 180)      + OFFSET_GAMMA); 
  Servo_3.write(map(potenziometro[3], 0, 1023, 100, 180)); 

}
  
