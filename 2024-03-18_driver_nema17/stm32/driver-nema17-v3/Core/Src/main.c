...
/* Configure the peripherals common clocks */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  long int json8 = ParserJSON("pps");   // pulses per second
	  ...
	  if ((json1 == device_id) || (json1 == DEVICE_ID_BROADCAST)) {
		  ...
		  if ( json8 != JSON_TOKEN_NOT_FOUND ) { tmpCsum += json8; }
	 	  ...
	 	  if ( csum == tmpCsum ) {
	 		  ...
	 		  // PPS (PULSES PER SECOND eseguo subito)
	 		  if ( json8 != JSON_TOKEN_NOT_FOUND ) {
	 			  pps = json8;
	 			  if (direction > 0) {
	 				 HAL_GPIO_WritePin(A4988_DIR_GPIO_Port,  A4988_DIR_Pin, 1);
	 			  }
	 		  ...
	 		  if (json1 == device_id) {
	 			  ...
	 			  sprintf((char *)txBuffer, "  {id:%d,deg:%d,pps:%d,ack:1,rnd:%d,csum:%d}\n ",
	 	  				              device_id, angolo4096, pps, rnd, device_id + angolo4096 + 1 + rnd );
	  ...
	  /* se PULSE PER SECOND != 0 gestisce il movimento */
	  if (pps > 0) {
		  /* gestione abbastanza rudimentale */
		  /* faccio uno step */
		  HAL_GPIO_WritePin(A4988_STEP_GPIO_Port, A4988_STEP_Pin, 1);
		  HAL_Delay( 1000 / pps );
		  HAL_GPIO_WritePin(A4988_STEP_GPIO_Port, A4988_STEP_Pin, 0);
